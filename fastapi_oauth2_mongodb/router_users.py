...
@router_users.post("/api/users/v1/register", status_code=status.HTTP_200_OK, response_model=RegisterResult)
async def api_users_v1_register(register_data: RegisterData):
    return await register(register_data)


@router_users.post("/api/users/v1/login", status_code=status.HTTP_200_OK, response_model=dict)
...
@router_users.get("/api/users/v1/current_user", status_code=status.HTTP_200_OK, response_model=dict)
